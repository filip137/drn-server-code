...
## Focused OM HWA prerequisite

The first DRN network pilot consumes only the OM adaptive,
lower-from-RESET, `tau/step=0.5`, cap-128 condition. Its separate immutable
study is
`studies/ibm-reram-om-hwa-device-model-20260822-v1.json`, backed by
`hwa_production_cap128_om_continuous.json` and
`hwa_production_cap128_om_corrupt.json`.

For each arm, one-pulse calibration runs on 204 calibration identities times
four repeats and 41 targets (33,456 trajectories). Adaptive programming runs
on all 1,024 identities times four repeats and 41 targets (167,936
trajectories). That gives 201,392 trajectories per arm and 402,784 total. The
upper boundary comes from the same fixed sampled identity for analysis-only
reachability classification; it is never exposed to the lower-start
controller. The broader cap-128 study remains the declared control for HfO2,
upper-start, and full-controller comparisons.
