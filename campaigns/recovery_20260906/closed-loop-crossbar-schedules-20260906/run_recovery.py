...
def schedule_factor(schedule, epoch):
    if epoch < 1 or schedule['kind'] not in {'constant', 'exponential'}:
        raise ValueError('Expected a positive epoch and constant or exponential schedule.')
    factor, horizon = schedule['final_factor'], schedule['decay_epochs']
    if not 0 < factor <= 1 or horizon < 2 or (schedule['kind'] == 'constant' and factor != 1):
        raise ValueError('Expected 0 < final_factor <= 1 and decay_epochs >= 2; constant factor must be 1.')
    return factor ** min((epoch - 1) / (horizon - 1), 1.0)
...
def run(args):
    ...
    schedule = plan['schedules'][args.schedule]
    schedule_factor(schedule, 1)
    out = args.output.resolve()
    out.mkdir(parents=True, exist_ok=False)
    start = time.time()
    status = {'status': 'initializing', 'pid': os.getpid(), 'started': start, 'architecture': args.architecture, 'condition': args.condition, 'writer': args.writer, 'schedule': args.schedule, 'heartbeat': start}
    ...
    try:
        ...
        manifest.update(schedule_name=args.schedule, learning_rate_schedule=schedule,
                        verify_tolerance=adapter.tolerance,
                        maximum_verify_pulses=plan['maximum_verify_pulses_per_update'],
                        total_pulse_cap=plan['total_pulse_cap'])
        for name in ('training/ibm_om_standard_crossbar.py', 'experiments/mnist_analog_relu/staged_runtime.py'):
            manifest['code_sha256'][name] = file_hash(HERE / 'crossbar_code' / name)
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            optimizer.learning_rate_scale = schedule_factor(schedule, epoch)
            effective_lr = lr * optimizer.learning_rate_scale
            loss_sum, examples, writer_time = 0., 0, 0.
            stream = hashlib.sha256()
            input_stream, label_stream = hashlib.sha256(), hashlib.sha256()
            for batch, (inputs, labels) in enumerate(islice(adapter.data.train, args.maximum_batches), 1):
                ...
                input_stream.update(inputs.numpy().tobytes())
                label_stream.update(labels.numpy().tobytes())
                ...
                if batch == 1 or batch % 100 == 0:
                    status.update(status='running', epoch=epoch, batch=batch, pulses=total_pulses, heartbeat=time.time(), writer_seconds=writer_time, effective_learning_rate=effective_lr)
                    write_json(out / 'status.json', status)
            ...
            record.update(effective_learning_rate=effective_lr,
                          ordered_model_inputs_sha256=input_stream.hexdigest(),
                          ordered_labels_sha256=label_stream.hexdigest(),
                          apparent_sha256=tensor_hash(adapter.apparent()),
                          persistent_sha256=tensor_hash(adapter.persistent()),
                          pulsed_cells=int((optimizer.pulse_count > 0).sum()),
                          maximum_pulses_per_cell=int(optimizer.pulse_count.max()))
            if args.writer == 'closed_loop_pv':
                error = (closed.target - adapter.apparent()).abs()
                record['target_error_max'] = float(error.max())
                record['target_error_rms'] = float(error.double().square().mean().sqrt())
...
if __name__ == '__main__':
    ...
    parser.add_argument('--architecture', choices=['crossbar'], default='crossbar')
    parser.add_argument('--schedule', choices=['constant', 'exponential'], required=True)
