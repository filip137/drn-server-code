...
def run(args):
    ...
    try:
        ...
        spec, teacher, runtime = engine._runtime(config, teacher_path=Path(plan['drn'].get('teacher_override', prepared['teacher']['path'])), gain=float(prepared['fixed_logit_gain']), smoke=False)
        stack = runtime['stack']
