...
def build_parser() -> argparse.ArgumentParser:
    ...
    recovery_readout = commands.add_parser('recovery-readout', help='Read-only replay of a saved recovery state')
    recovery_readout.add_argument('--config', type=Path, required=True)
    recovery_readout.add_argument('--teacher-weights', type=Path, required=True)
    recovery_readout.add_argument('--device-state', type=Path, required=True)
    recovery_readout.add_argument('--split', choices=['validation','test'], default='validation')
    recovery_readout.add_argument('--output', type=Path, required=True)
...
def _dispatch(
    ...
    *,
    ...
    if args.command_name == 'recovery-readout':
        from experiments.mnist_analog_relu.recovery_readout import run
        return run(args)
