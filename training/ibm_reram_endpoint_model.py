...
def sample_ibm_reram_endpoints(
    ...
    *,
    ...
    Passing the returned mask back through ``corrupt_mask`` preserves which
    sites are defective, but this marginal endpoint model does not preserve a
    corrupt site's latent stuck value or pulse history. Repeated physical
    writes must continue from an ``IbmReramPlant`` state instead.
    """
    ...
    corrupt_persistent_endpoint = _sample_quantile_model(
        records,
        lower_index,
        upper_index,
        fraction,
