...
class IbmReramAcceptedEndpointSample:
    """One accepted, healthy apparent-endpoint draw for population HWA.

    Persistent state, reachability failures, and corrupt identities are
    intentionally absent.  Those are separate physical-deployment outcomes;
    this sample is only the target-conditioned programming-error realization
    used by an IBM-style off-chip HWA forward.
    """
...
class IbmReramAcceptedEndpointModel:
    """Compiled accepted-endpoint residual density for population HWA."""
    ...
    def to(
        ...
        *,
        ...
        """Move the compiled tables without changing their fingerprint."""
...
def build_ibm_reram_accepted_endpoint_model(
    ...
    *,
    condition_key: str,
    expected_coordinate: str | None = None,
    allow_inadequate: bool = False,
) -> IbmReramAcceptedEndpointModel:
    """Compile the accepted healthy residual rows of a v2 endpoint fit.

    The result is the device-population programming-error component used by
    HWA.  It intentionally omits fixed identities, bound tuples, reference
    values, corrupt masks, failure branches, and persistent pulse state.
    """
    ...
    coordinate_value = artifact.get("coordinate", "unspecified")
    if not isinstance(coordinate_value, str) or not coordinate_value:
        raise ValueError("Expected the endpoint artifact coordinate to be text.")
    if (
        expected_coordinate is not None
        and coordinate_value != expected_coordinate
    ):
        raise ValueError(
            "Expected the endpoint artifact coordinate to match the HWA "
            f"conductance coordinate {expected_coordinate!r}. Provided value: "
            f"{coordinate_value!r}."
        )
    target_grid = torch.tensor(
    ...
    digest.update(coordinate_value.encode("utf-8"))
    for name, value in (
