...
| IBM optimized-material ReRAM array | `ReRamArrayOMPresetDevice`; `ibm_reram_program_verify.v1`; `ibm_reram_om_program_verify` modifier | Normalized pulsed state nominally in `[-1,1]`, fixed sampled cell bounds and corruption identity, pulse-resolved or held-out-calibrated endpoint execution | Cap-128 pulse characterization, off-chip HWA, physical selection, and persistent deployment handoff | Hardware-derived fitted model |
...
| Figure-6-informed HfO2 endpoint regimes | `hfo2_figure6_endpoint_regimes.py`; `figure6_ratio_independent` or `figure6_ratio_rank_matched` | Unitless positive synthetic endpoints: RESET centred at `0.1`, lower-clipped at `0.05`, and assigned the Figure-6 RESET/SET deviation ratio; SET centred at `2.0` with unchanged HfO2 upper-bound deviation and no clipping | Exploratory endpoint-distribution, direct DRN initialization, endpoint-noise HWA, apparent-feedback program-and-verify deployment, and hybrid pulse-recovery controls | Idealized or analyst-selected model |
| Figure-6 endpoints + IBM-OM pulse dynamics | `figure6_om_pulse.py`; `figure6_om_pulse_corruption_experiment.py` | The same synthetic Figure-6 RESET/SET conductance endpoints, with sampled AIHWKit 1.1.0 OM `dwmin_up/down`, soft-bounds response, cycle/write noise, and paired published-corrupt versus repaired identities | Matched direct/HWA P&V and open-loop pulse-recovery comparison; OM native bound locations are replaced by the Figure-6 endpoints | Hybrid analyst-selected endpoint / hardware-derived fitted pulse model |
