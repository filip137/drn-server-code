...
def _append_scale_transform(transforms_list, scale):
    """Append a scaling transform when scale != 1.0."""
    if abs(scale - 1.0) > 1e-9:
        transforms_list.append(torchvision.transforms.Lambda(lambda t, s=scale: t * s))


def load_mnist(normalize=False, augment_32x32=False, normalize_std=1.0, data_root="data"):
    ...
    if normalize:
        ...
        _append_scale_transform(training_transforms, normalize_std)
        _append_scale_transform(test_transforms, normalize_std)
...
def load_kuzushiji_mnist(normalize=False, augment_32x32=False, normalize_std=1.0, data_root="data"):
...
def load_fashion_mnist(normalize=True, augment_32x32=False, normalize_std=1.0, data_root="data"):
...
def load_svhn(normalize=True, normalize_std=1.0, data_root="data"):
    ...
    if normalize:
        transforms_after_tensor = [torchvision.transforms.Normalize(mean, std)]
        _append_scale_transform(transforms_after_tensor, normalize_std)
        final_transform = torchvision.transforms.Compose(transforms_after_tensor)
    else:
...
def load_cifar10(normalize=True, normalize_std=1.0, data_root="data"):
...
def load_cifar100(normalize=True, normalize_std=1.0, data_root="data"):
...
def load_dataset(dataset, normalize=True, augment_32x32=False, normalize_std=1.0, data_root="data"):
    ...
    if dataset == 'MNIST': return load_mnist(normalize, augment_32x32, normalize_std, data_root)
    elif dataset == 'KuzushijiMNIST': return load_kuzushiji_mnist(normalize, augment_32x32, normalize_std, data_root)
    elif dataset == 'FashionMNIST': return load_fashion_mnist(normalize, augment_32x32, normalize_std, data_root)
    elif dataset == 'SVHN': return load_svhn(normalize, normalize_std, data_root)
    elif dataset == 'CIFAR10': return load_cifar10(normalize, normalize_std, data_root)
    elif dataset == 'CIFAR100': return load_cifar100(normalize, normalize_std, data_root)
    else: raise ValueError("expected 'MNIST', 'KuzushijiMNIST', 'FashionMNIST', 'SVHN', 'CIFAR10' or 'CIFAR100' but got {}".format(dataset))


def load_dataloaders(dataset, batch_size, augment_32x32=False, normalize=True, normalize_std=1.0, data_root="data"):
