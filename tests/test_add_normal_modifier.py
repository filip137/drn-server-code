...
def _invalid_states(valid):
    ...
    inconsistent_seed = copy.deepcopy(valid)
    inconsistent_seed["resolved_seed"] = 61
    cases.append(inconsistent_seed)
...
def test_load_state_dict_rejects_malformed_state_without_partial_mutation():
    ...
    for invalid in _invalid_states(valid):
        with pytest.raises(ValueError, match="Expected"):
