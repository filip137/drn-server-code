...
CONFIG = ROOT / "examples/mnist_relu_drn/measured_raw_single_finetune_10ep.json"
CAMPAIGN = (
    ROOT / "campaigns/manifests/mnist_relu_drn_initialized_single_10ep.json"
)
...
def test_initialized_single_config_freezes_the_ten_epoch_protocol() -> None:
    ...
    assert train.settings.learning_rates == (2.1e-10, 5.7e-13)
...
def test_initialized_single_campaign_has_train_then_explicit_test() -> None:
    payload = json.loads(CAMPAIGN.read_text(encoding="utf-8"))
    assert payload["campaign_id"] == (
        "mnist-single-reram-initialized-finetune-10ep-20260817-v1"
    )
    assert [stage["id"] for stage in payload["stages"]] == [
        "initialized_single_train",
        "initialized_single_test",
    ]
    train, test = payload["stages"]
    assert train["command"] == "train"
    assert test["command"] == "validate"
    assert test["depends_on"] == ["initialized_single_train"]
    assert test["inputs"]["weights"] == {
        "stage": "initialized_single_train",
        "artifact_kind": "selected_named_weights",
    }


def test_single_topology_is_model_local_across_repeated_construction() -> None:
    ...
    assert _single_topology_signature(
        first_report, voltage_amp=4.0, current_amp=0.25
    ) == expected
    assert _single_topology_signature(
        second_report, voltage_amp=4.0, current_amp=0.25
    ) == expected
...
def test_checkpoint_topology_validation_ignores_names_but_rejects_semantics() -> None:
    ...
    mutated["amplification_indices"][1]["forward_gain"] = 1.0
