...
def plot_all_hidden(summary_path):
    summary_path = Path(summary_path)
    with summary_path.open("r", encoding="utf-8") as f:
        data = json.load(f)
    ...
    hidden = data.get("hidden", {})
    if not hidden:
        raise ValueError("No hidden keys found in summary JSON.")

    all_metadata = []
    for hidden_key in sorted(hidden.keys()):
        out_png = summary_path.with_name(f"p90_vs_rel_tol_{hidden_key}.png")
        out_svg = summary_path.with_name(f"p90_vs_rel_tol_{hidden_key}.svg")
        metadata_path = summary_path.with_name(f"p90_vs_rel_tol_{hidden_key}_metadata.json")
        metadata = plot_p90_vs_rel_tol(summary_path, hidden_key, out_png, out_svg)
        with metadata_path.open("w", encoding="utf-8") as f:
            json.dump(metadata, f, indent=2)
        all_metadata.append(metadata)

    all_metadata_path = summary_path.with_name("p90_vs_rel_tol_metadata_all.json")
...
def plot_all_hidden_one_plot(summary_path, output_path, output_svg=None):
    summary_path = Path(summary_path)
    output_path = Path(output_path)
    if output_svg is not None:
        output_svg = Path(output_svg)
    ...
    for hidden_key in sorted(hidden.keys()):
        ...
        ax.plot(rel_tols, p90s, marker="o", linewidth=1.6, label=hidden_key)
        metadata_hidden[hidden_key] = {"rel_tol": rel_tols, "p90": p90s}

    ax.set_xscale("log")
    ax.set_yscale("log")
    ax.set_xlabel("rel_tol")
    ax.set_ylabel("p90 error")
    ax.set_title("p90 vs rel_tol (all hidden)")
    ax.grid(True, which="both", linestyle="--", linewidth=0.6, alpha=0.5)
...
def main():
    ...
    parser.add_argument("--all-hidden", action="store_true", help="Plot all hidden keys to separate files")
    parser.add_argument("--all-hidden-one", action="store_true", help="Plot all hidden keys on one figure")
    ...
    if args.all_hidden:
        if args.hidden_key or args.out_png or args.out_svg or args.metadata_path:
            raise ValueError("--all-hidden cannot be combined with --hidden/--out/--out-svg/--metadata.")
        plot_all_hidden(summary_path)
        return

    if args.all_hidden_one:
        if args.hidden_key or args.out_png or args.out_svg or args.metadata_path:
            raise ValueError("--all-hidden-one cannot be combined with --hidden/--out/--out-svg/--metadata.")
        out_png = summary_path.with_name("p90_vs_rel_tol_all_hidden.png")
        out_svg = summary_path.with_name("p90_vs_rel_tol_all_hidden.svg")
        metadata_path = summary_path.with_name("p90_vs_rel_tol_all_hidden_metadata.json")
        metadata = plot_all_hidden_one_plot(summary_path, out_png, out_svg)
        with metadata_path.open("w", encoding="utf-8") as f:
            json.dump(metadata, f, indent=2)
        return
