#!/usr/bin/env python3
"""Plot Single Shockley and Double Shockley 100-epoch MNIST comparisons."""
...
DEFAULT_STEM = "mnist_single20_double_shockley20_100epoch_accuracy"
...
SERIES = (
    SeriesSpec(
        key="single_shockley_20",
        label="Single Shockley diode, 20 outputs",
        color="#59a14f",
        paths=(
            MNIST_ROOT
            / "single_diode_exponential_output_dim_probe_20260515"
            / "runs"
            / "output_dim_20"
            / "seed_0"
            / "20260515-095648_train-stats"
            / "time_series.pkl",
            MNIST_ROOT
            / "single_diode_exponential_output_dim_probe_20260515"
            / "runs"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot MNIST 100-epoch accuracy for Single and Double Shockley runs."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def write_summary(rows: list[dict[str, object]], output_paths: list[Path], csv_path: Path, path: Path) -> None:
    payload = {
        "title": "MNIST 100-epoch Single/Double Shockley comparison",
