...
from experiments.validate_conv13_zero_bias_adam_eqprop_read_noise_5em4_pack import (
    runtime_beta_values,
    scientific_outcome,
)
...
WRAPPER = (
    ROOT
    / "experiments/run_conv13_zero_bias_adam_eqprop_read_noise_5em4_jeanzay.slurm"
)


def test_frozen_plan_has_six_cases_in_three_two_run_packs() -> None:
    study = load_and_validate_study(DEFAULT_STUDY)
    payload = plan(study)

    assert payload["case_count"] == 6
    assert payload["pack_count"] == 3
    assert payload["packs"] == [[0, 1], [2, 3], [4, 5]]
    assert PACKS == ((0, 1), (2, 3), (4, 5))
    assert [row["index"] for row in payload["cases"]] == list(range(6))
    ...
    assert payload["production_array"] == "0-2%3"
...
def test_declared_noise_draws_follow_batches_epochs_and_state_layers() -> None:
    ...
    assert draws["smoke"] == {"conv1": 4, "conv3": 8}
    assert draws["production"]["conv1"] == 3438 * 10 * 4
    assert draws["production"]["conv3"] == 3438 * 30 * 8
...
def test_study_rejects_changed_source_and_parent_hashes(tmp_path: Path) -> None:
    ...
    changed_source["source"]["configs"]["conv1"]["baseline"]["sha256"] = "0" * 64
...
def test_slurm_wrapper_freezes_umg_packing_canary_and_production() -> None:
    ...
    required = (
        ...
        "experiments.validate_conv13_zero_bias_adam_eqprop_read_noise_5em4_pack",
        "C13RN_ENVIRONMENT_PASS",
        "C13RN_SOURCE_PASS",
        "C13RN_SEMANTIC_PASS",
...
def test_canary_summary_is_complete_but_production_is_scientific_terminal() -> None:
    validator = (
        ROOT
        / "experiments/validate_conv13_zero_bias_adam_eqprop_read_noise_5em4_pack.py"
