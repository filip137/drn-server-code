#!/usr/bin/env python3
"""Persistently supervise the fail-closed perfect-diode successor launch."""
...
class SuccessorJeanZaySupervisor:
    ...
    def initialize(self) -> None:
        ...
        binding = {
            ...
            "official_verifier_path": str(
                Path(self.args.official_verifier).expanduser().resolve()
            ),
        }
    ...
    def run(self, *, poll_seconds: float, once: bool) -> dict[str, Any]:
        if (
        ...
        while True:
            ...
            if (
                once
                or result["status"] in {"complete", "submission_required"}
            ):
                return result
