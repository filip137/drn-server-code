#!/usr/bin/env python3
"""Run a clean hard-sigmoid Conv BP gradient sweep over separate T and K."""
...
WEIGHT_PREFIXES = ("weight0", "weight1", "weight2")
...
def _param_by_name(row: dict) -> dict[str, dict]:
    raw = row.get("param_summaries_json") or "[]"
...
def _add_weight_columns(row: dict, eps: float) -> dict:
    by_name = _param_by_name(row)
    out = dict(row)
    statuses = []
    for index, prefix in enumerate(WEIGHT_PREFIXES):
        summary = by_name.get(f"ConvWeight_{index}")
        if summary is None and index == 2:
            summary = by_name.get("DenseWeight_0")
    ...
    w0, w1 = statuses[0], statuses[1]
    if w0 == "alive" and w1 == "alive":
        early = "alive"
    elif w0 in {"dead", "zero"} and w1 in {"dead", "zero"}:
        early = "dead"
    else:
        early = "partial"
    out["early_conv_status"] = early
    return out
...
def _write_markdown(path: Path, rows: list[dict]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    lines = [
        "# Hard-Sigmoid Conv2 T/K Gradient Sweep",
        "",
        "Status is based on shared ConvWeight parameter gradients after spatial reduction.",
        "",
    ]
    ...
    for (run_name, phase), group in sorted(grouped.items()):
        lines.extend(
            [
                f"## {run_name} / {phase}",
                "",
                "| T | K | early | W0 L2 | W0 zero | W1 L2 | W1 zero | Dense L2 | h1 sat | h2 sat | global rel |",
                "| ---: | ---: | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
            ]
        )
        for row in sorted(group, key=lambda item: (int(item["T"]), int(item["K"]))):
            lines.append(
                "| {T} | {K} | {early} | {w0} | {z0} | {w1} | {z1} | {w2} | {h1} | {h2} | {glob} |".format(
                    T=row["T"],
                    K=row["K"],
                    early=row["early_conv_status"],
                    ...
                    w2=_fmt(row["weight2_grad_l2"]),
                    h1=_fmt(row["hidden1_saturation"]),
                    h2=_fmt(row["hidden2_saturation"]),
...
def main() -> None:
    ...
    for spec in specs:
        if spec.model_cfg.get("non_linearity") != "hard_sigmoid":
            raise ValueError(f"Expected hard_sigmoid run, got {spec.model_cfg.get('non_linearity')}: {spec.run_dir}")
        for phase in phases:
            for T in args.T_values:
                for K in args.K_values:
                    ...
                    rows.append(_add_weight_columns(row, args.eps))
