...
OUTPUT_ROOT="${OUTPUT_ROOT:-${REPO_ROOT}/results/mnist_bp_conv_inputgain_train_sweep_hardsigmoid_voff15_seed0_5epoch_lr_scaled}"
...
scaled_lr() {
  local input_gain="$1"
  awk -v base_lr="${BASE_LR}" -v input_gain="${input_gain}" -v base_gain="${BASE_GAIN}" \
    'BEGIN { printf "%.12g", base_lr * input_gain / base_gain }'
}
...
run_job() {
  ...
  lr="$(scaled_lr "${input_gain}")"
  lr_label="$(label_float "${lr}")"
  local job_root="${OUTPUT_ROOT}/conv${conv_depth}/input_gain_${gain_label}/lr_${lr_label}/k_${k_label}"

  echo "[lane ${LANE_INDEX}/${NUM_LANES}] job=${index} conv${conv_depth} ${run_name} input_gain=${input_gain} K=${iterations} lr=${lr} output=${job_root}"
