#!/usr/bin/env bash
# One A100 read-noise/control case per Slurm task. SOURCE CONFIGS OUTPUT DATA [smoke].
set -euo pipefail
(( $# == 4 || $# == 5 )) || exit 2
mode=${5:-production}
[[ $mode == production || $mode == smoke || $mode == benchmark ]] || exit 2
source_dir=$1
config_dir=$2
output_root=$3
data_dir=$4
...
# Throughput check uses the highest-noise baseline case, excluded from production.
[[ $mode != benchmark ]] || config="$config_dir/${configs[11]}"
extra=()
[[ $mode != smoke ]] || extra+=(--smoke)
[[ $mode != benchmark ]] || extra+=(--epoch-override 1)
