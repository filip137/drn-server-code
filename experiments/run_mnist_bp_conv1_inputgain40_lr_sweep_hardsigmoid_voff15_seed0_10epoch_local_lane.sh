...
run_job() {
  ...
  echo "[lane ${LANE_INDEX}/${NUM_LANES}] job=${index} ${run_name} input_gain=40 K=4 lr=${lr} output=${job_root}"
...
for lr in 0.003 0.006 0.009 0.012 0.018 0.024; do
  run_job "${job_index}" mnist_bp_amp_v1_c1 "${lr}"
  job_index=$((job_index + 1))
done

for lr in 0.003 0.006 0.009 0.012 0.018; do
  run_job "${job_index}" mnist_bp_amp_v2_c1 "${lr}"
  job_index=$((job_index + 1))
done

for lr in 0.003 0.006 0.009 0.012 0.018; do
  run_job "${job_index}" mnist_bp_amp_v4_c1 "${lr}"
  job_index=$((job_index + 1))
done

for lr in 0.006 0.009 0.012 0.018 0.024 0.036; do
  run_job "${job_index}" mnist_bp_amp_v1_c2 "${lr}"
  job_index=$((job_index + 1))
done

for lr in 0.006 0.009 0.012 0.018 0.024 0.036 0.048; do
  run_job "${job_index}" mnist_bp_amp_v1_c4 "${lr}"
  job_index=$((job_index + 1))
done
