...
OUTPUT_ROOT="${OUTPUT_ROOT:-${REPO_ROOT}/results/mnist_bp_conv1_hardsigmoid_noamp_saturation_targets_voff2p5_seed0_10epoch_lr0p5}"
...
if [[ -n "${WAIT_FOR_PID}" ]]; then
  while ps -p "${WAIT_FOR_PID}" >/dev/null 2>&1; do
    {
      date
      echo "waiting for pid ${WAIT_FOR_PID} before starting v_off=2.5 lr0p5 queue"
...
# Same initialized no-amplification Conv1 v_off=2.5 saturation targets as the
# LR x2 setup, with learning rates now using lr = 0.36 / input_gain.
run_job 0.10 16.524959564208984 0.021785227286105765 sat10
run_job 0.30 41.03604507446289 0.008772775235692275 sat30
run_job 0.50 75.98670196533203 0.004737671075186885 sat50
run_job 0.70 111.79586029052734 0.0032201550134723852 sat70
run_job 0.90 292.7659606933594 0.0012296511491548056 sat90
