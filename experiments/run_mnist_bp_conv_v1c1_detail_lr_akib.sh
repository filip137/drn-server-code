...
OUTPUT_ROOT="${OUTPUT_ROOT:-${REPO_ROOT}/results/mnist_bp_conv_fixed_step_v1c1_detail_lr_akib_20260704}"
...
PY
  ...
  job_root="${OUTPUT_ROOT}/conv2/${nonlinearity}/mnist_bp_amp_v1_c1/input_gain_${gain_label}/T_${T}_K_${K}/lr_center_$(label_float "${center_lr}")/lr_factor_${factor_label}/lr_${lr_label}"

  echo "[akib-v1c1-detail] $(date -Is) lane=${LANE} nonlinearity=${nonlinearity} factor=${factor} lr=${learning_rate} root=${job_root}"
...
case "${LANE}" in
  hs)
    # R3 tested 0.5, 1, 2 and the best row was still improving at factor 2.
    input_gain="${HS_INPUT_GAIN:-245.00401306152344}"
    center_lr="${HS_CENTER_LR:-0.0235098189945}"
    T="${HS_T:-6}"
    K="${HS_K:-16}"
    read -r -a factors <<< "${HS_FACTORS:-1.5 2 2.5 3 3.5 4}"
    for factor in "${factors[@]}"; do
      run_train hard_sigmoid "${factor}" "${input_gain}" "${center_lr}" "${T}" "${K}"
    done
    ;;
  pd)
    # R3 factor 2 had the best accuracy but final loss/accuracy already softened,
    # so probe around the shoulder rather than only increasing the LR.
    input_gain="${PD_INPUT_GAIN:-100}"
    center_lr="${PD_CENTER_LR:-0.012}"
    T="${PD_T:-6}"
    K="${PD_K:-4}"
    read -r -a factors <<< "${PD_FACTORS:-1.25 1.5 1.75 2 2.25 2.5 3}"
    for factor in "${factors[@]}"; do
      run_train perfect_diode "${factor}" "${input_gain}" "${center_lr}" "${T}" "${K}"
    done
    ;;
  *)
    echo "Expected LANE=hs or pd, got ${LANE}." >&2
    exit 2
    ;;
esac
