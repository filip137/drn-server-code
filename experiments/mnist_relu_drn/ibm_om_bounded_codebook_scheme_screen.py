...
def load_screen_contract(path: Path) -> ScreenContract:
    ...
    if raw["schema_version"] != CONTRACT_SCHEMA_VERSION:
        raise ValueError("Expected screen schema_version to equal 1.")
    ...
    scales_raw = raw["scale_fractions"]
    if not isinstance(scales_raw, list) or not scales_raw:
        raise ValueError("Expected a non-empty scale_fractions list.")
    scales = tuple(
        _finite_fraction(
            value,
            name=f"scale_fractions[{index}]",
            include_zero=False,
            include_one=True,
        )
    ...
    if not isinstance(selection, dict):
        raise ValueError("Expected selection to be an object.")
    _exact_keys(selection, {"domain", "primary", "tie_break"}, label="selection")
    if selection != {
        "domain": "development_assignment_training_calibration_subset",
        "primary": "bounded_codebook_student_accuracy",
        "tie_break": "bounded_codebook_calibrated_kl_then_lexicographic_scales",
    }:
        raise ValueError("Expected the predeclared development-only selection rule.")
...
def _calibration_report(
    ...
    *,
    ...
    report = fit_positive_logit_gain(
        raw_scores,
        teacher_logits,
        gain_min=gain_min,
        gain_max=gain_max,
    ...
    if student_prediction.shape != labels.shape:
    ...
    *,
    ...
    for topology in (4, 8):
        ...
        for scheme in topology_schemes:
            candidates = []
            for pair in product(contract.scale_fractions, repeat=2):
                exact, _continuous, mapping = build_bounded_scheme_targets(
                    logical_weights,
                    population,
                    codebook,
                    scheme=scheme,
                ...
                candidates.append(
                    {
                        "scale_fractions": [float(pair[0]), float(pair[1])],
                        "bounded_codebook_calibration": calibration,
                        "mapping": mapping,
                    }
                )
            selected_index = min(
                ...
                key=lambda index: (
                    -candidates[index]["bounded_codebook_calibration"]["student_accuracy"],
                    candidates[index]["bounded_codebook_calibration"]["calibrated_kl"],
                    tuple(candidates[index]["scale_fractions"]),
                ),
            )
            winner = candidates[selected_index]
            pair = tuple(float(value) for value in winner["scale_fractions"])
            ...
            continuous_calibration = _calibration_report(
                stack,
                teacher,
                loaders.calibration,
                calibration_labels,
            ...
            selected[scheme.name] = {
                ...
                "bounded_codebook_gain": winner["bounded_codebook_calibration"]["gain"],
                "continuous_envelope_gain": continuous_calibration["gain"],
                "bounded_codebook_calibration": winner["bounded_codebook_calibration"],
                "continuous_envelope_calibration": continuous_calibration,
                "selected_mapping": mapping,
                "candidates": candidates,
            }
            print(
                f"selected {scheme.name}: scales={pair}, "
                f"calibration_accuracy="
                f"{100.0 * winner['bounded_codebook_calibration']['student_accuracy']:.2f}%",
                flush=True,
            )
    ...
    for assignment_seed in contract.heldout_assignment_seeds:
        for topology in (4, 8):
            ...
            for scheme in (
                ...
                frozen = selected[scheme.name]
                pair = tuple(float(value) for value in frozen["scale_fractions"])
                exact, continuous, mapping = build_bounded_scheme_targets(
                    logical_weights,
                    population,
                    codebook,
                ...
                apply_targets(stack.bundle.catalog, exact)
                stack.cost.gain = float(frozen["bounded_codebook_gain"])
                ...
                apply_targets(stack.bundle.catalog, continuous)
                stack.cost.gain = float(frozen["continuous_envelope_gain"])
