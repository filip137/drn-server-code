#!/usr/bin/env python3
"""Materialize and run the protocol-matched Conv2 Adam EqProp sigma-5e-4 study."""
...
DEFAULT_STUDY = (
    ROOT
    / "configs/conv/perfectdiode_conv2_zero_bias_adam_eqprop_one_decade_"
    "sigma_5em4_ordinary_mnist_30ep_seed0_20260817_v1.json"
)
SCHEMA = "perfectdiode-conv2-zero-bias-adam-eqprop-one-decade-read-noise-study/v1"
STUDY_ID = (
    "perfectdiode-conv2-zero-bias-adam-centered-float64-eqprop-one-decade-"
    "sigma-5em4-ordinary-mnist-30ep-seed0-20260817-v1"
)
...
# Baseline and ours share one V100; legacy occupies the second V100.  This is
# the measured two-runs-per-GPU packing contract requested for Conv2.
CASE_SPECS = (
    CaseSpec(0, "conv2", "baseline"),
    CaseSpec(1, "conv2", "ours"),
    CaseSpec(2, "conv2", "legacy"),
)
PACKS = ((0, 1), (2,))
...
def load_and_validate_study(path: Path) -> dict[str, Any]:
    ...
    expected_contract = {
        ...
        "T": {"conv2": 6},
        "K": {"conv2": 6},
        "output_row_exponent_L": {"conv2": 2},
        "injected_beta_B": {
            "conv2": {"baseline": 100.0, "ours": 10.0, "legacy": 0.03},
        },
        "base_beta": {
            "conv2": {"baseline": 100.0, "ours": 0.625, "legacy": 0.0001171875},
        },
    ...
    if (
        gradient.get("passing_layer_batch_rows") != 72
        or gradient.get("total_layer_batch_rows") != 72
        or gradient.get("all_rows_passed") is not True
    ...
    expected_execution = {
        "target": "jean-zay",
        "canary_array": "0-0",
        "production_array": "0-1%2",
        "logical_case_count": 3,
        "pack_count": 2,
        "runs_per_gpu": 2,
        "cpus_per_task": 16,
    ...
    if tuple(spec.index for spec in CASE_SPECS) != tuple(range(3)):
    ...
    for scheme in SCHEMES:
        digest = study["source"]["clean_controls"]["result_sha256"]["conv2"][
            scheme
        ]
    ...
    config["qualification_source"].update(
        {
            ...
            "gradient_gate_passed_rows": 72,
            "gradient_gate_total_rows": 72,
            "gradient_gate_scope": "conv2_all_schemes_both_checkpoints_four_batches",
            "gradient_gate_override": None,
            "conv3_t8_residual_deviation": None,
            "paper_ready": False,
        }
    )
    return config
