#!/usr/bin/env python3
"""Validate one packed protocol-matched Conv2 Adam EqProp sigma-5e-4 task."""
...
def validate_pack(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.execution_mode != expected_execution_mode:
        raise ValueError(
            f"Pack {pack_index} must use {expected_execution_mode!r} execution."
        )
    ...
    if len(config_paths) != len(CASE_SPECS):
        raise ValueError(f"Expected three configs, found {len(config_paths)}.")
    ...
    for logical_index in logical_indices:
        ...
        checks: dict[str, bool] = {
            ...
            == 72,
            "gradient gate complete": qualification.get(
                "gradient_gate_total_rows"
            )
            == 72,
        ...
        receipt = {
            "schema_version": "conv2-zero-bias-adam-eqprop-read-noise-run-receipt/v1",
    ...
    pack_receipt = {
        "schema_version": "conv2-zero-bias-adam-eqprop-read-noise-pack-receipt/v1",
        "recorded_at": datetime.now(timezone.utc).isoformat(),
        "study_id": STUDY_ID,
        ...
        "concurrent_on_one_gpu": expected_execution_mode == "concurrent",
        "job_id": args.job_id,
        "parent_job_id": args.parent_job_id,
        "runs": receipts,
    }
