...
OUTPUT_ROOT="${OUTPUT_ROOT:-${REPO_ROOT}/results/mnist_bp_conv1_hardsigmoid_noamp_saturation_targets_seed0_10epoch_lr0p5}"
...
if [[ -n "${WAIT_FOR_PID}" ]]; then
  while ps -p "${WAIT_FOR_PID}" >/dev/null 2>&1; do
    {
      date
      echo "waiting for pid ${WAIT_FOR_PID} before starting v_off=1.5 lr0p5 queue"
...
run_job() {
  ...
  echo "START target=${target} input_gain=${input_gain} lr=${learning_rate} v_off=1.5 job_root=${job_root}" | tee -a "${OUTPUT_ROOT}/logs/queue.log"
  wait_for_gpu
  ...
  echo "DONE target=${target} input_gain=${input_gain} lr=${learning_rate} v_off=1.5" | tee -a "${OUTPUT_ROOT}/logs/queue.log"
}

# Same initialized no-amplification Conv1 saturation targets as the baseline
# v_off=1.5 run, with every baseline learning rate multiplied by 0.5.
run_job 0.10 10.2362 0.0351692805 sat10
run_job 0.30 24.845 0.014489854 sat30
run_job 0.50 45.8423 0.0078530115 sat50
run_job 0.70 67.3022 0.005349004 sat70
run_job 0.90 176.104 0.0020442522 sat90
