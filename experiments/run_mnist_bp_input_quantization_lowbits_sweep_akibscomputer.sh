...
OUTPUT_ROOT="/home/filip/server_code/results/mnist_bp_input_quantization_sweep_standard_best_bits2_3_4_8"
...
export MPLCONFIGDIR="/tmp/matplotlib-mnist-input-quantization-lowbits"

echo "$(date) launching MNIST BP low-bit input-quantization sweep" | tee -a "${LOG_DIR}/queue.log"
...
for shard in $(seq 0 14); do
  log_path="${LOG_DIR}/input_quant_lowbits_shard_${shard}.log"
...
if [ "${status}" -eq 0 ]; then
  echo "$(date) low-bit input-quantization sweep finished successfully" | tee -a "${LOG_DIR}/queue.log"
else
  echo "$(date) low-bit input-quantization sweep finished with errors" | tee -a "${LOG_DIR}/queue.log"
fi
