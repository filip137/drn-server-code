#!/usr/bin/env python3
"""Validate one packed Conv1/2/3 beta-collapse task and write receipts."""
...
def validate_pack(args: argparse.Namespace) -> dict[str, Any]:
    ...
    expected_epochs = 1 if args.run_mode == "smoke" else 10
    ...
    if len(config_paths) != len(CASE_SPECS):
        raise ValueError(f"Expected 36 configs, found {len(config_paths)}.")
    ...
    provenance_rows: list[tuple[str, str, str, tuple[str, ...]]] = []
    ...
    for logical_index in logical_indices:
        ...
        checks: dict[str, bool] = {
            ...
            "optimizer": config.get("optimizer", {}).get("name") == spec.optimizer,
        ...
        provenance_rows.append(
            (
                str(provenance["train_indices_sha256"]),
                str(provenance["validation_indices_sha256"]),
                str(provenance["first_epoch_batch_order_sha256"]),
                train_orders,
            )
        )
        initial_hashes.append(initial_hash)
        receipt = {
            "schema_version": "conv123-eqprop-beta-collapse-run-receipt/v1",
    ...
    if len(set(provenance_rows)) != 1:
        raise ValueError("Dataset split or minibatch order differs within the pack.")
    if CASE_SPECS[logical_indices[0]].architecture == "conv2" and len(
        set(initial_hashes)
    ) != 1:
        raise ValueError("Matched Conv2 boundary/control initialization differs.")

    pack_receipt = {
        "schema_version": "conv123-eqprop-beta-collapse-pack-receipt/v1",
        "recorded_at": datetime.now(timezone.utc).isoformat(),
        "study_id": STUDY_ID,
    ...
    semantic_summary = [
        {
            ...
            "collapsed_count": sum(row["outcome"]["collapsed"] for row in receipts),
            "execution_mode": args.execution_mode,
            "concurrent_on_one_gpu": args.execution_mode == "concurrent",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        required=True,
        help="Observed execution mode for the two logical runs in this pack.",
    )
