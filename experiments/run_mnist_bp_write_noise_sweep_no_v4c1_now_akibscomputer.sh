...
OUTPUT_ROOT="/home/filip/server_code/results/mnist_bp_write_noise_sweep_standard_best_30x_no_v4c1"
...
export MPLCONFIGDIR="/tmp/matplotlib-mnist-write-noise-no-v4c1"

echo "$(date) launching MNIST BP write-noise sweep excluding v4/c1" | tee -a "${LOG_DIR}/queue.log"
...
for shard in $(seq 0 11); do
  log_path="${LOG_DIR}/write_noise_no_v4c1_shard_${shard}.log"
...
if [ "${status}" -eq 0 ]; then
  echo "$(date) write-noise no-v4/c1 sweep finished successfully" | tee -a "${LOG_DIR}/queue.log"
else
  echo "$(date) write-noise no-v4/c1 sweep finished with errors" | tee -a "${LOG_DIR}/queue.log"
fi
